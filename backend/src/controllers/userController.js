...
async function createUser(req, res) {
    try {
        ...
        if (req.user && (req.user.role === "administrativo" || req.user.role === "gerencia" || req.user.role === "encargado")) {
...
async function getUsers(req, res) {
    try {
        ...
        if (filterRole && (role === "administrativo" || role === "gerencia" || role === "encargado")) {
            queryFilter = { 
...
async function updateUser(req, res) {
    try {
        ...
        // Admin, gerencia and encargado restrictions
...
async function deleteUserAdmin(req, res) {
    try {
        ...
        // Solo gerencia, RRHH y encargado pueden eliminar usuarios
        if (req.user.role !== "gerencia" && req.user.role !== "RR.HH" && req.user.role !== "encargado") {
...
async function deleteUser(req, res) {
    try {
        ...
        if (req.user.role !== "gerencia" && req.user.role !== "encargado") {
            return res.status(403).json({ error: "Acceso denegado" });
        }
