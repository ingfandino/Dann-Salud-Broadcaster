...
exports.listLogs = async (req, res) => {
    try {
        ...
        logger.error("❌ Error listando logs:", err);
...
exports.exportLogsJSON = async (_req, res) => {
    try {
        ...
        logger.error("❌ Error exportando logs JSON:", err);
        res.status(500).json({ error: err.message });
    }
};
...
exports.exportLogsCSV = async (_req, res) => {
    try {
        ...
        logger.error("❌ Error exportando logs CSV:", err);
        res.status(500).json({ error: err.message });
    }
};
...
exports.exportLogsExcel = async (_req, res) => {
    try {
        ...
        logger.error("❌ Error exportando logs Excel:", err);
        res.status(500).json({ error: err.message });
    }
};