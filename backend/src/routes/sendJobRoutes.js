...
logger.info("🚦 [sendJobRoutes] Recibida petición POST /send-jobs/start", { headers: req.headers, body: req.body });
    ...
    requireAuth,
    permit("asesor", "supervisor", "admin"),
...
// Controles
router.post("/:id/pause", requireAuth, permit("supervisor", "admin"), async (req, res) => {
logger.info(`🚦 [sendJobRoutes] POST /send-jobs/${req.params.id}/pause`);
    await sendJobController.pauseJob(req, res);
});
router.post("/:id/resume", requireAuth, permit("supervisor", "admin"), async (req, res) => {
...
router.delete("/:id/cancel", requireAuth, permit("supervisor", "admin"), async (req, res) => {
    logger.info(`🚦 [sendJobRoutes] DELETE /send-jobs/${req.params.id}/cancel`);
    await sendJobController.cancelJob(req, res);
});

// Consultas
router.get("/:id", requireAuth, permit("asesor", "supervisor", "admin"), async (req, res) => {
    logger.info(`🚦 [sendJobRoutes] GET /send-jobs/${req.params.id}`);
    await sendJobController.getJob(req, res);
});
router.get("/", requireAuth, permit("asesor", "supervisor", "admin"), async (req, res) => {
