...
/**
 * Middleware para verificar rol Gerencia o Encargado
 */
const gerenciaOrEncargado = (req, res, next) => {
    const role = req.user?.role?.toLowerCase();
    if (role !== 'gerencia' && role !== 'encargado') {
        return res.status(403).json({
            success: false,
            message: 'Acceso denegado. Esta funcionalidad es exclusiva para Gerencia y Encargados.'
        });
    }
    next();
};
 ...
 */
router.get('/', requireAuth, gerenciaOrEncargado, controller.listSeparations);
 ...
 * @access  Gerencia only
 */
router.get('/:id', requireAuth, gerenciaOrEncargado, controller.getSeparationById);
 ...
 * @access  Gerencia only
 */
router.patch('/:id/mark-paid', requireAuth, gerenciaOrEncargado, controller.markAsPaid);
 ...
 * @access  Gerencia only
 */
router.patch('/:id/motivo-baja', requireAuth, gerenciaOrEncargado, controller.updateMotivoBaja);
