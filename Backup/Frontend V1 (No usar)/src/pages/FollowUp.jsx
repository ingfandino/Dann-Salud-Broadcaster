...
export default function FollowUp() {
    ...
    const formatDateTime = (value) => {
        ...
        const formattedTime = date.toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "UTC" });
    ...
    const fetchAudits = async () => {
        try {
            ...
            // ✅ FIX: Solo ocultar estados exactos "Cargada" y "Aprobada", NO "Aprobada, pero no reconoce clave"
            ...
            const normalizedFilteredAudits = hasExplicitStatusFilter
                ...
                : auditsArray.filter((audit) => {
                    ...
                    // ✅ Comparación exacta para evitar ocultar "Aprobada, pero no reconoce clave"
    ...
    const fetchFilterOptions = async () => {
        try {
            ...
            const asesoresRes = await apiClient.get('/users', {
                // ✅ Item 8: Mostrar todos los asesores
                params: {}
            });
            ...
            const auditoresRes = await apiClient.get('/users', {
                // ✅ Item 8: Mostrar todos los auditores
                params: { includeAllAuditors: 'true' }
            });
            ...
            // ✅ Item 8: Mostrar todos los grupos, sin filtrar por supervisor
                                                ...
                                                {audit.scheduledAt
                                                    ? (
                                                        <span className="inline-flex items-center px-0.5 py-0.5 rounded text-[8px] md:text-[11px] font-semibold bg-blue-100 text-blue-800">
                                                            {new Date(audit.scheduledAt).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit", timeZone: "UTC" })}
                                                        </span>
                                                    )
                                                    : "-"}
                                            </td>
                                                ...
                                                {audit.scheduledAt
                                                    ? (
                                                        <span className="inline-flex items-center px-0.5 py-0.5 rounded text-[8px] md:text-[11px] font-semibold bg-indigo-100 text-indigo-800">
                                                            {new Date(audit.scheduledAt).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit", hour12: false, timeZone: "UTC" })}
                                                        </span>
                                                    )
                                                    : "-"}
                                            </td>
