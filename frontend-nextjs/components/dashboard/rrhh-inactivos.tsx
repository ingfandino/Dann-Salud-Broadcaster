...
export function RRHHInactivos() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: any = { ...safeFields, fotoDNI: fotoDNIPath }
      // Solo Gerencia puede modificar la fecha de egreso
      if (isGerencia && fechaBaja) {
        payload.fechaBaja = fechaBaja
      }
      await api.employees.update(selectedEmpleado._id, payload)
                  ...
                  {teamEmployees.map((emp) => (
                    ...
                    >
                      ...
                      <td className={cn("px-3 py-3 max-w-[150px] truncate", theme === "dark" ? "text-gray-400" : "text-gray-600")}>
                        {emp.motivoBajaNormalizado || emp.motivoBaja || "-"}
                      </td>
                      <td className="px-3 py-3">
                        <span
                            ...
                            <>
                              ...
                              <button
                                onClick={async () => {
                                  ...
                                  try {
                                    await api.employees.reactivate(emp._id);
              ...
              <div className="space-y-4 overflow-y-auto flex-1 pr-2">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className={cn(
                    ...
                    Inactivo (usar botón de reactivar en la tabla)
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => isGerencia && setEditForm({ ...editForm, fechaBaja: e.target.value })}
                    disabled={!isGerencia}
                    className={cn(
                      "w-full px-3 py-2 rounded-lg border text-sm",
                      !isGerencia
