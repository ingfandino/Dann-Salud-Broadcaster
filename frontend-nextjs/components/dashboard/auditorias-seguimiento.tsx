...
const getStatusColor = (status: string, theme: string) => {
  ...
  const colors: Record<string, { light: string; dark: string }> = {
    ...
    "qr hecho (temporal)": { light: "bg-green-200 text-green-800", dark: "bg-green-500/20 text-green-400" },
    "hacer qr": { light: "bg-gray-200 text-gray-700", dark: "bg-gray-500/20 text-gray-400" },
    "aprobada": { light: "bg-teal-600 text-white", dark: "bg-teal-500/30 text-teal-300" },
    "cargada": { light: "bg-sky-100 text-sky-800", dark: "bg-sky-500/20 text-sky-400" },
...
const getRowBackgroundByStatus = (status: string, theme: string) => {
  ...
  const colorMap: Record<string, { light: string; dark: string }> = {
    ...
    "autovinculación": { light: "bg-pink-50 hover:bg-pink-100", dark: "bg-pink-900/20 hover:bg-pink-900/30" },
...
export function AuditoriasSeguimiento() {
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      /* Obtener supervisores (incluye Gerencia y Encargado para consistencia con modal de edición) */
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 text-center">
                      <div className="flex items-center justify-center gap-1 transition-opacity">
                        {/* ✅ Recuperador y Administrativo ven el modal en modo SOLO LECTURA */}
                        {!isAsesor && (
                          <button
                        ...
                        {/* Recuperador y Encargado NO pueden eliminar ventas */}
                        {(isAdmin || isGerencia) && !isRecuperador && !isEncargado && (
                          <button
