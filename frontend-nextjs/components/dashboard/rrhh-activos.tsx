  ...
  const handleSave = async () => {
    ...
    if (isDeactivating) {
      ...
      if (!editForm.motivoBajaNormalizado) {
        toast.error("Debe seleccionar un motivo de baja para desactivar al empleado.")
        return
      }
      if (!editForm.fechaBaja) {
        toast.error("Debe indicar la fecha de egreso para desactivar al empleado.")
        return
      }
    }
    ...
    try {
      ...
      await api.employees.update(selectedEmpleado._id, { ...safeFields, fotoDNI: fotoDNIPath })

      // ── Segundo: si se desactiva, llamar al endpoint dedicado ──
      if (isDeactivating) {
        await api.employees.deactivate(selectedEmpleado._id, {
                        ...
                        <input
                          ...
                          className={cn(
                            ...
                            theme === "dark"
                              ? "bg-white/5 border-white/10 text-white"
                              : "bg-white border-gray-200 text-gray-800",
                            missingFechaEgreso && "border-red-500",
                          )}
                        />
                        {missingFechaEgreso && (
                          <p className="text-xs text-red-500 mt-1">La fecha de egreso es obligatoria para dar de baja.</p>
                        )}
                      </div>
                      <div>
                        <label
                        ...
                        {missingMotivo && (
                          <p className="text-xs text-red-500 mt-1">Debe seleccionar un motivo de baja.</p>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex justify-end gap-3 mt-6 flex-shrink-0">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    (saving || deactivationIncomplete) ? "opacity-50 cursor-not-allowed" : "hover:opacity-90"
                  )}
