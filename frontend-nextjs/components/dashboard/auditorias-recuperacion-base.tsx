...
import {
  Filter, Download, Pencil, Trash2, X, ChevronDown, RefreshCw, CheckCircle
} from "lucide-react"
...
export function AuditoriasRecuperacionBase({ type }: AuditoriasRecuperacionBaseProps) {
  ...
  const [supervisorFilterInitialized, setSupervisorFilterInitialized] = useState(false)
  ...
  /* Estado del popover de revisión */
  const [revisionPopoverId, setRevisionPopoverId] = useState<string | null>(null)
  const revisionPopoverRef = useRef<HTMLDivElement>(null)
  ...
  const fetchAudits = async () => {
    try {
      ...
      if (config.requireDisponibleParaVenta) {
        auditsArray = auditsArray.filter((audit: Audit) => 
          audit.status?.toLowerCase() === 'afip' || audit.disponibleParaVenta === true
        )
      }
  ...
  const handleMarkRevision = async (auditId: string, listaParaReventa: boolean) => {
    try {
      const { data } = await api.audits.markRevision(auditId, { lista_para_reventa: listaParaReventa })
      setAudits(prev => prev.map(a => a._id === auditId ? { ...a, ultima_revision: data.ultima_revision, lista_para_reventa: data.lista_para_reventa, revisado_por: data.revisado_por } : a))
      setRevisionPopoverId(null)
      toast.success(listaParaReventa ? "Marcado como revisado y listo para reventa" : "Marcado como revisado")
    } catch (err) {
      console.error(err)
  ...
  /* Auto-seleccionar supervisor actual si el usuario es supervisor */
  useEffect(() => {
    if (isSupervisor && user?.nombre && supervisoresList.length > 0) {
      ...
      if (supervisorActual && !selectedSupervisores.includes(supervisorActual.nombre)) {
        setSelectedSupervisores([supervisorActual.nombre])
      }
  ...
  }, [isSupervisor, user, supervisoresList])

  /* Carga inicial de auditorías - solo después de inicializar filtro de supervisor */
  useEffect(() => {
  ...
  useEffect(() => {
    if (!supervisorFilterInitialized) return
    fetchAudits()
  }, [filters, selectedSupervisores, selectedAsesores, dateFrom, dateTo, sortOrder, supervisorFilterInitialized])
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (revisionPopoverRef.current && !revisionPopoverRef.current.contains(event.target as Node)) {
        setRevisionPopoverId(null)
      }
    }
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-3">
          ...
          <div className="relative" ref={supervisorFilterRef}>
            ...
            {isSupervisorDropdownOpen && !isSupervisor && (
          ...
          <table className="w-full text-xs">
            <thead className={cn(
              ...
              <tr>
                ...
                <th className="px-2 py-2 w-[100px]">Última rev.</th>
                <th className="px-2 py-2 w-[70px] text-center">Reventa</th>
                <th className="px-2 py-2 w-[100px] text-center">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={13} className="px-4 py-8 text-center opacity-50">
                    ...
                    <td className="px-2 py-1.5 text-center">
                      {item.lista_para_reventa ? (
                        ...
                        <span className={cn(
                          ...
                          theme === "dark" ? "bg-gray-500/20 text-gray-400" : "bg-gray-100 text-gray-500"
                        )}>No</span>
                      )}
                    </td>
                    <td className="px-2 py-1.5 text-center">
                      <div className="flex items-center justify-center gap-1 transition-opacity relative">
                        {canMarkRevision && (
                          <div className="relative" ref={revisionPopoverId === item._id ? revisionPopoverRef : undefined}>
                            <button
                              onClick={() => setRevisionPopoverId(revisionPopoverId === item._id ? null : item._id)}
                              className={cn(
                                "p-1 rounded",
                                item.ultima_revision
                                  ? "hover:bg-green-100 text-green-600 dark:hover:bg-green-900/30 dark:text-green-400"
                                  : "hover:bg-gray-100 text-gray-400 dark:hover:bg-gray-700 dark:text-gray-500"
                              )}
                              title="Marcar revisión"
                            >
                              <CheckCircle className="w-3.5 h-3.5" />
                            </button>
                            {revisionPopoverId === item._id && (
                              <div className={cn(
                                "absolute right-0 bottom-full mb-1 z-50 w-48 rounded-lg shadow-lg border p-2 space-y-1",
                                ...
                                <button
                                  ...
                                  className={cn(
                                    "w-full text-left px-2 py-1.5 rounded text-xs font-medium transition-colors",
                                    theme === "dark" ? "hover:bg-green-900/30 text-green-400" : "hover:bg-green-50 text-green-700"
                                  )}
                                >
                                  Revisado + Reventa
                                </button>
