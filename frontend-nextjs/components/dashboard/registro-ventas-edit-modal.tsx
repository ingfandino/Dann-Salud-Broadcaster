  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.fechaQR) {
        ...
        (form.status === "QR hecho" && audit.status !== "QR hecho") ||
        (form.status === "QR hecho pero pendiente de aprobación" && audit.status !== "QR hecho pero pendiente de aprobación")
      ) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={cn(
                      "w-full px-3 py-2 rounded-lg border text-sm",
                      (form.status !== 'QR hecho' && form.status !== 'QR hecho pero pendiente de aprobación' && !isGerencia) && "opacity-50 cursor-not-allowed",
                      theme === "dark" ? "bg-white/5 border-white/10 text-white" : "bg-white border-gray-200 text-gray-800"
                    )}
                  />
                  <p className={cn("text-xs mt-1", theme === "dark" ? "text-gray-400" : "text-gray-500")}>
                    {(form.status !== 'QR hecho' && form.status !== 'QR hecho pero pendiente de aprobación' && !isGerencia)
                      ? "Se habilita en estado 'QR hecho' o 'QR hecho pero pendiente de aprobación' (o Gerencia)"
