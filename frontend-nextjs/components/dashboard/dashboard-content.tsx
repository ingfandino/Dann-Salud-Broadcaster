...
export function DashboardContent({ activeSection, onSectionChange }: DashboardContentProps) {
  ...
  const renderContent = () => {
    ...
    if (activeSection === "auditorias-reventa") {
      // Acceso: Supervisor, Encargado, Gerencia
      const role = user?.role?.toLowerCase()
      const allowedRoles = ['gerencia', 'encargado', 'supervisor']
    ...
    if (activeSection === "rrhh-bajo-rendimiento") {
      ...
      if (role !== 'gerencia' && role !== 'encargado') {
        return (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            <p className="text-red-500 text-lg font-semibold">⛔ Acceso Denegado</p>
            <p className="text-gray-500 mt-2">Esta sección es exclusiva para Gerencia y Encargados.</p>
          </div>
        )
      }
