...
export function Evidencias() {
  ...
  useEffect(() => {
    setPage(1)
  }, [estadoFilter, fechaDesde, fechaHasta, selectedAdmins])

  // Fetch admin users for filter
  useEffect(() => {
    async function loadAdminUsers() {
      try {
        const res = await api.users.list('role=administrativo')
        const list = Array.isArray(res.data) ? res.data : (res.data?.users || [])
        const admins: AdminUser[] = list
          .filter((u: any) => u.nombre && u.nombre.trim().length > 0)
          .map((u: any) => ({ _id: u._id, nombre: u.nombre }))
          .sort((a: AdminUser, b: AdminUser) => a.nombre.localeCompare(b.nombre))
    ...
    try {
      ...
      if (estadoFilter === "en_proceso" || estadoFilter === "terminada") {
        ...
        const [ventasResult, evidenciasResult] = await Promise.all([
          ...
          evidenciasService.getEvidencias(evidenciasParams)
        ])
        ...
        const combined: FilaEvidencia[] = ventas.map((venta) => {
          ...
          return {
            ...
            administrativo: (venta as any).administrador?.nombre || null,
            administrativoId: (venta as any).administrador?._id || null,
          }
        })
  ...
  }, [searchDebounced, searchCuilDebounced, estadoFilter, fechaDesde, fechaHasta, page, selectedAdmins])
    ...
    <div className="space-y-6 animate-fade-in-up relative">
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-3">
          ...
          {/* Filtro Administrativo - Multi-select */}
          <div className="relative" ref={adminDropdownRef}>
            ...
            >
              <span className={cn("truncate", selectedAdmins.length === 0 && (theme === "dark" ? "text-gray-500" : "text-gray-400"))}>
                {selectedAdmins.length === 0
                  ? "Administrativo"
                  : selectedAdmins.length === 1
                    ? adminUsers.find(a => a._id === selectedAdmins[0])?.nombre || "1 seleccionado"
                    : `${selectedAdmins.length} seleccionados`}
              </span>
              <svg className={cn("w-4 h-4 flex-shrink-0 transition-transform", adminDropdownOpen && "rotate-180")} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {adminDropdownOpen && (
              <div className={cn(
                "absolute z-50 mt-1 w-full max-h-60 overflow-auto rounded-lg border shadow-lg",
                theme === "dark"
                ...
                <div className={cn(
                  "flex items-center justify-between px-3 py-1.5 border-b text-xs font-medium",
                  theme === "dark" ? "border-white/10 text-gray-300" : "border-gray-100 text-gray-600"
                )}>
                  <button
                    type="button"
                    onClick={() => setSelectedAdmins(adminUsers.map(a => a._id))}
                    className={cn("hover:underline", theme === "dark" ? "text-teal-400" : "text-teal-600")}
                  >
                    Seleccionar todos
                  </button>
                  <button
                    ...
                    className={cn("hover:underline", theme === "dark" ? "text-red-400" : "text-red-500")}
                  >
                    Limpiar
                  </button>
                </div>
                {adminUsers.length === 0 ? (
                  <div className={cn("px-3 py-2 text-xs", theme === "dark" ? "text-gray-500" : "text-gray-400")}>
                    No hay administrativos
                  </div>
                ) : (
                  adminUsers.map((admin) => (
                    ...
                    >
                      ...
                      />
                      <span className="truncate">{admin.nombre}</span>
                    </label>
                  ))
                )}
              </div>
          ...
          <table className="w-full text-xs">
            <thead className={cn(
              ...
              <tr>
                ...
                <th className="px-2 py-2.5 text-center w-[120px]">Administrativo</th>
                ...
                filas.map((f) => (
                  ...
                  >
                    ...
                    <td className={cn("px-2 py-2.5 text-center", theme === "dark" ? "text-gray-300" : "text-gray-600")}>
                      {f.administrativo ? (
                        <span className="truncate max-w-[110px] inline-block" title={f.administrativo}>
                          {f.administrativo}
                        </span>
                      ) : (
      ...
      {uploadModalOpen && createPortal(
        <div
          ref={uploadModalRef}
          className={cn(
            "fixed right-4 z-[9999] w-[400px] rounded-2xl p-6 border-2 animate-[modalPop_0.2s_ease-out]",
            theme === "dark"
