...
export function AuditoriasCrearTurno() {
  ...
  const [elianaSoledad, setElianaSoledad] = useState<any>(null)
  ...
  useEffect(() => {
    if ((isSupervisor || isEncargado) && ventaPropia) {
      ...
      if (elianaSoledad) {
        setForm(prev => ({ ...prev, supervisor: elianaSoledad._id }))
  ...
  }, [ventaPropia, isSupervisor, isEncargado, user, elianaSoledad])
  ...
  const loadElianaSoledadForIndependiente = async () => {
    try {
      ...
      if (eliana) {
        ...
        toast.error('No se pudo encontrar a Eliana Soledad (Gerencia) en el sistema')
      }
    } catch (error) {
  ...
  }, [otroEquipo, form.asesor, form.supervisor, user, isSupervisor, isAsesor, isRecuperador, isGerenciaOrAuditorSinEquipo, perteneceOtroSupervisor, supervisorSeleccionado, supervisores, elianaSoledad])
  ...
  useEffect(() => {
    if (isRecuperador && validadores.length > 0 && !form.validador) {
      ...
      const elianaValidador = validadores.find((v: any) =>
        ...
        v.nombre?.toLowerCase().includes('soledad')
      )
      if (elianaValidador) {
  ...
  const loadSupervisores = async () => {
    try {
      ...
      const eliana = users.find((s: any) =>
        ...
        s.nombre?.toLowerCase().includes('soledad')
      )
      if (eliana) {
        setElianaSoledad(eliana)
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <SearchableSocialHealthSelect
              ...
              onChange={(value) => handleChange("obraSocialAnterior", value)}
          ...
          {/* Supervisor fijo para Recuperador (solo Eliana Soledad) */}
          {isRecuperador && (
            <div className={cn(
              "p-3 rounded-lg border",
              theme === "dark" ? "bg-purple-500/10 border-purple-500/30" : "bg-purple-50 border-purple-200"
              ...
              <p className={cn("text-sm", theme === "dark" ? "text-purple-400" : "text-purple-700")}>
                👑 <strong>Supervisor:</strong> {elianaSoledad?.nombre || "Eliana Soledad (cargando...)"}
              </p>
              <p className={cn("text-xs mt-1", theme === "dark" ? "text-purple-400/70" : "text-purple-600")}>
                La venta se asignará automáticamente al equipo de Eliana Soledad
              </p>
            </div>
          )}
