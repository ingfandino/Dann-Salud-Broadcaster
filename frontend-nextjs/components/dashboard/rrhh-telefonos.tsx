...
export function RRHHTelefonos() {
    ...
    const [filterAsesoresList, setFilterAsesoresList] = useState<UserOption[]>([])
    ...
    const [independientes, setIndependientes] = useState<UserOption[]>([])

    /* ========== CARGA DE DATOS ========== */
    const loadPhones = useCallback(async (filters?: { supervisor?: string; asesor?: string; numeroTelefono?: string; estado?: string }) => {
        try {
            setLoading(true)
            const [phonesRes, statsRes] = await Promise.all([
                api.phones.list(filters),
                api.phones.getStats()
            ])
    ...
    const loadIndependientes = useCallback(async () => {
        try {
            const res = await api.phones.getIndependientes()
    ...
    const loadFilterAsesores = useCallback(async () => {
        try {
            const res = await api.phones.getAsesoresForFilter()
    ...
    useEffect(() => {
        ...
        loadFilterAsesores()
    }, [loadSupervisors, loadIndependientes, loadFilterAsesores, isGerencia, isEncargado])

    /* ========== FILTRADO (server-side) ========== */
    // Efecto unificado: aplica filtros con debounce corto para texto, inmediato para dropdowns
    useEffect(() => {
        const timeout = setTimeout(() => {
            const filters: { supervisor?: string; asesor?: string; numeroTelefono?: string; estado?: string } = {}
    ...
    }, [filterSupervisor, filterAsesor, filterNumero, filterEstado, loadPhones])
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="sin-recarga">Sin recargas</option>
            ...
            {loading ? (
                ...
                <div className={cn(
                    ...
                    <p className={theme === "dark" ? "text-gray-400" : "text-gray-600"}>
                        {hasActiveFilters ? "No se encontraron teléfonos con los filtros aplicados" : "No hay teléfonos registrados"}
                    </p>
                </div>
            ) : (
...
function PhoneModal({ theme, isGerencia, isEncargado, supervisors, asesores, independientes, user, phone, onClose, onSave, loadAsesores }: {
    theme: string
    isGerencia: boolean
    isEncargado: boolean // ✅ NUEVO: Rol Encargado
    supervisors: UserOption[]
    asesores: UserOption[]
    independientes: UserOption[]
    user: any
    phone?: PhoneData
    onClose: () => void
    ...
    }, [form.supervisor, form.esAsesorIndependiente, form.usoPropio, isGerencia, isEncargado, supervisors, loadAsesores])
        ...
        >
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Check: Uso propio (visible para Gerencia, Supervisor, Independiente) */}
                    {canSeeUsoPropio && (
                    ...
                    {/* Check: Es asesor independiente (solo visible para Gerencia, deshabilitado si usoPropio) */}
                    {isGerencia && (
                        <div className="flex items-center gap-3">
                            <input
                                type="checkbox"
                                ...
                                className={cn(
                                    ...
                                    form.usoPropio && "opacity-50 cursor-not-allowed"
                                )}
                            />
                            <label
                    ...
                    {(isGerencia || isEncargado) ? (
                        <div>
                            <label className={cn(
                                ...
                                Supervisor {!form.usoPropio && !form.esAsesorIndependiente ? "*" : ""}
                            </label>
                            <select
                                ...
                                className={cn(
                                    ...
                                    theme === "dark"
                                        ? "bg-white/5 border-white/10 text-white"
                                        : "bg-white border-gray-200 text-gray-900",
                                    (form.usoPropio || form.esAsesorIndependiente) && "opacity-50 cursor-not-allowed"
                                )}
                            >
                                <option value="">{form.usoPropio ? "No aplica (Uso propio)" : form.esAsesorIndependiente ? "No aplica (Independiente)" : "Seleccione supervisor"}</option>
                                {!form.usoPropio && !form.esAsesorIndependiente && supervisors.map(s => (
                                    <option key={s._id} value={s._id}>
                    ...
                    <div>
                        <label className={cn(
                            ...
                            {form.usoPropio ? "Asesor (uso propio)" : form.esAsesorIndependiente ? "Asesor independiente" : "Asesor asignado"}
                        </label>
                        <select
                        ...
                        >
                            {form.usoPropio ? (
                                <option value={user?._id || ""}>{user?.nombre || "Usuario actual"}</option>
                            ) : (
                                <>
                                    <option value="">Sin asignar</option>
                                    {(form.esAsesorIndependiente ? independientes : asesores).map(a => (
                                        <option key={a._id} value={a._id}>
                                            {a.nombre}
                                        </option>
                                    ))}
